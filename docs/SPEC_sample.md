...
* 言語: Swift 5.8+（プロジェクト開始時の最新安定版に合わせる）
* SwiftUI: Xcode 13+/SwiftUI 3+ 相当の API を利用可能
...
* `public struct SidebarView: View` — 組み込みのサイドバーコンポーネント（カスタマイズポイント多数）
...
* macOS では `SidebarListStyle()` を基本にし、選択時のフォーカスやアクセントカラーを AppKit に近づける。
...
* iPadOS 側では外部メニューやフォーカスのふるまいが macOS と異なるため、プラットフォームごとの最適化コードを用意する。
...
1. 0.1.0 — コアモデル + SidebarView（読み取り専用、展開/折り畳み、選択）
2. 0.2.0 — アイコン・バッジ・カスタム行、検索
3. 0.3.0 — 編集（rename）、コンテキストメニュー
4. 0.4.0 — ドラッグ＆ドロップ、多重選択の強化
5. 1.0.0 — ドキュメント整備・安定版リリース
...
この仕様書は「最初の提案」です。詳細設計（関数シグネチャ、具体的な型定義、API サンプルコード、DocC コメント方針）まで落とし込んだドキュメント作成も可能です。次はどのくらい詳細に落とし込みましょうか？
